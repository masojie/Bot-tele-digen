...
class BotDetector {
    ...
    classifyActivity(botSignals, aiSignals, riskScore) {
        if (botSignals >= 3 || riskScore >= 0.60)
            return 'BOT';
        if (aiSignals >= 2 || (botSignals >= 1 && aiSignals >= 1))
            return 'AI';
        return 'HUMAN';
    }
    getActivityDisplay(type) {
        switch (type) {
            case 'BOT':
    ...
    async checkTxInterval(mint) {
        try {
            const sigs = await this.connection.getSignaturesForAddress(new web3_js_1.PublicKey(mint), { limit: 20 });
            if (sigs.length < 5)
    ...
    async checkHolders(mint) {
        ...
        try {
            ...
            if (accounts.length === 0)
                return { risk: 0.1, flags, greenFlags };
